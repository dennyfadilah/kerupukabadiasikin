...
function kerupukMentah()
{
    global $koneksi;

    $result = mysqli_query($koneksi, "SELECT * FROM kerupuk_mentah");

    $idlasted = mysqli_query($koneksi, "SELECT id_mth FROM kerupuk_mentah ORDER BY id_mth DESC LIMIT 1");
    $rowIdLasted = mysqli_fetch_array($idlasted);

    if ($result) {
        require_once "views/kerupukMentah.php";
    } else {
        echo mysqli_error($koneksi);
    }
}

function addDataKerupukMentah()
{
    global $koneksi;
    ...
    // Saldo
    $sqlSaldo = mysqli_query($koneksi, "SELECT saldo_qty AS sqty, saldo_total AS stotal FROM kerupuk_mentah ORDER BY id_mth DESC LIMIT 1");
...
function editDataKerupukMentah($id_mth)
{
    ...
    // Saldo
    $query = mysqli_query($koneksi, "SELECT saldo_qty AS sqty, saldo_total AS stotal FROM kerupuk_mentah ORDER BY id_mth DESC LIMIT 1,1");
    ...
    $result = mysqli_query($koneksi, "UPDATE kerupuk_mentah SET 
    tanggal = '$tanggal', 
...
function deleteDataKerupukMentah($id_mth)
{
    global $koneksi;

    $result = mysqli_query($koneksi, "DELETE FROM kerupuk_mentah WHERE id_mth = $id_mth");
